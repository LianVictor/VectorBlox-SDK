...
if [ -f squeezenet1_0 ]; then
   if ! echo "0ad906b637e800c3adb9abc4602d6261 squeezenet1_0" | md5sum -c; then
       echo -e "\n There is an issue with the torchvision_squeezenet1_0 model file as the expected checksum does not match.\n The model source can be found at: https://pytorch.org/vision/0.14/models/generated/torchvision.models.squeezenet1_0.html#torchvision.models.squeezenet1_0.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f torchvision_squeezenet1_0.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t torchvision_squeezenet1_0.pre.tflite  -o torchvision_squeezenet1_0_V1000_ncomp.vnnx
fi
...
if [ -f torchvision_squeezenet1_0_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py torchvision_squeezenet1_0_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model torchvision_squeezenet1_0_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
