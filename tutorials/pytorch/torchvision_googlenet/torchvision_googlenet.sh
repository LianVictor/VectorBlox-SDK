...
if [ -f googlenet.onnx ]; then
   if ! echo "c7d42f2545b5cd5181d25d3db3c73914 googlenet.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the torchvision_googlenet model file as the expected checksum does not match.\n The model source can be found at: https://pytorch.org/vision/0.14/models/googlenet.html.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f torchvision_googlenet.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t torchvision_googlenet.pre.tflite  -o torchvision_googlenet_V1000_ncomp.vnnx
fi
...
if [ -f torchvision_googlenet_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py torchvision_googlenet_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model torchvision_googlenet_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
