...
if [ -f public/mobilenet-v1-1.0-224/mobilenet-v1-1.0-224.caffemodel ]; then
   if ! echo "3fd6889ec48bda46451d67274144e2a8 public/mobilenet-v1-1.0-224/mobilenet-v1-1.0-224.caffemodel" | md5sum -c; then
       echo -e "\n There is an issue with the mobilenet-v1-1.0-224 model file as the expected checksum does not match.\n The model source can be found at: https://github.com/openvinotoolkit/open_model_zoo/tree/2021.4.2/models/public/mobilenet-v1-1.0-224.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f mobilenet-v1-1.0-224.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t mobilenet-v1-1.0-224.pre.tflite  -o mobilenet-v1-1.0-224_V1000_ncomp.vnnx
fi
...
if [ -f mobilenet-v1-1.0-224_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py mobilenet-v1-1.0-224_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model mobilenet-v1-1.0-224_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
