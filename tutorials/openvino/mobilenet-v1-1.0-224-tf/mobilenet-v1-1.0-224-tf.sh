...
echo "Checking for mobilenet-v1-1.0-224-tf files..."
...
if [ -f public/mobilenet-v1-1.0-224-tf/mobilenet_v1_1.0_224_frozen.pb ]; then
   if ! echo "7f564c782f0141e4e7bdc5161a6cc2f4 public/mobilenet-v1-1.0-224-tf/mobilenet_v1_1.0_224_frozen.pb" | md5sum -c; then
       echo -e "\n There is an issue with the mobilenet-v1-1.0-224-tf model file as the expected checksum does not match.\n The model source can be found at: https://github.com/openvinotoolkit/open_model_zoo/tree/2021.4.2/models/public/mobilenet-v1-1.0-224-tf/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f mobilenet-v1-1.0-224-tf.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t mobilenet-v1-1.0-224-tf.pre.tflite  -o mobilenet-v1-1.0-224-tf_V1000_ncomp.vnnx
fi
...
if [ -f mobilenet-v1-1.0-224-tf_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py mobilenet-v1-1.0-224-tf_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model mobilenet-v1-1.0-224-tf_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
