...
echo "Checking for yolov9-s_512x288_argmax files..."
...
if [ ! -f v9-s.onnx ]; then
    ...
    if [ -d YOLO ]; then
       ...
       python yolo/lazy.py task=export model=v9-s image_size=[288,512]
       cd ..
       deactivate
...
if [ ! -f yolov9-s_512x288_argmax.tflite ]; then
...
--output_integer_quantized_tflite
   cp saved_model/v9-s.cut_full_integer_quant.tflite yolov9-s_512x288_argmax.tflite
fi
...
if [ -f yolov9-s_512x288_argmax.tflite ]; then
   tflite_preprocess yolov9-s_512x288_argmax.tflite  --scale 255
fi
...
if [ -f yolov9-s_512x288_argmax.pre.tflite ]; then
   tflite_postprocess yolov9-s_512x288_argmax.pre.tflite   -p YOLO_ARG_MAX
fi
...
if [ -f yolov9-s_512x288_argmax.pre.post.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov9-s_512x288_argmax.pre.post.tflite  -o yolov9-s_512x288_argmax_V1000_ncomp.vnnx
fi
...
if [ -f yolov9-s_512x288_argmax_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov9-s_512x288_argmax_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.512.288.jpg -v 8 -t 0.3 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov9-s_512x288_argmax_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.512.288.jpg OBJECT_DETECT'
fi
