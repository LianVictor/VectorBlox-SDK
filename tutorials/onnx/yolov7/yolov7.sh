...
if [ -f v7.onnx ]; then
   if ! echo "ac3b62bdb8aae6b21c16bfd30ca10d40 v7.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the yolov7 model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ramonhollands/YOLO.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov7.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov7.pre.tflite  -o yolov7_V1000_ncomp.vnnx
fi
...
if [ -f yolov7_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov7_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 7 -t 0.3 -j yolov5n.json -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov7_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg YOLOV5'
fi
