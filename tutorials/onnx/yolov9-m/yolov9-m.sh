...
if [ ! -f v9-m.onnx ]; then
    ...
    if [ -d YOLO ]; then
       ...
       python yolo/lazy.py task=export model=v9-m image_size=[640,640]
       cd ..
       deactivate
       cp YOLO/weights/v9-m.onnx .
       source $VBX_SDK/vbx_env/bin/activate
    fi
fi

if [ ! -f v9-m.sim.onnx ]; then
   onnxsim v9-m.onnx v9-m.sim.onnx
fi
...
onnx.utils.extract_model("v9-m.sim.onnx", "v9-m.cut.onnx", ["images"], outputs)
EOF
fi
...
if [ -f yolov9-m.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov9-m.pre.tflite  -o yolov9-m_V1000_ncomp.vnnx
fi
...
if [ -f yolov9-m_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov9-m_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -t 0.3 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov9-m_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT'
fi
