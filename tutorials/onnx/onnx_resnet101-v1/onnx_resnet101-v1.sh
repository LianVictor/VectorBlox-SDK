...
echo "Checking for onnx_resnet101-v1 files..."
if [ ! -f onnx_resnet101-v1.tflite ]; then
# model details @ https://github.com/onnx/models/tree/main/validated/vision/classification/resnet
wget -q --no-check-certificate https://media.githubusercontent.com/media/onnx/models/main/validated/vision/classification/resnet/model/resnet101-v1-7.onnx
...
if [ -f resnet101-v1-7.onnx ]; then
   if ! echo "5603f2b5eeaa02640cb3e54861d60a24 resnet101-v1-7.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the onnx_resnet101-v1 model file as the expected checksum does not match.\n The model source can be found at: https://github.com/onnx/models/tree/main/validated/vision/classification/resnet.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
-i resnet101-v1-7.onnx \
--output_signaturedefs \
--output_integer_quantized_tflite
   cp saved_model/resnet101-v1-7_full_integer_quant.tflite onnx_resnet101-v1.tflite
fi
...
if [ -f onnx_resnet101-v1.tflite ]; then
   tflite_preprocess onnx_resnet101-v1.tflite  --mean 123.675 116.28 103.53 --scale 58.4 57.1 57.38
fi
...
if [ -f onnx_resnet101-v1.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t onnx_resnet101-v1.pre.tflite  -o onnx_resnet101-v1_V1000_ncomp.vnnx
fi
...
if [ -f onnx_resnet101-v1_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py onnx_resnet101-v1_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model onnx_resnet101-v1_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
