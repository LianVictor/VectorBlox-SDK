...
echo "Checking for yolov9-t_512x288_argmax files..."
...
model.export(format="onnx", output_path="v9-t.onnx", imgsz=(288,512))
EOF
       deactivate
       cd ..
...
if [ ! -f v9-t.sim.onnx ]; then
   onnxsim v9-t.onnx v9-t.sim.onnx --overwrite-input-shape "1,3,288,512"
fi
...
if [ ! -f yolov9-t_512x288_argmax.tflite ]; then
...
--output_integer_quantized_tflite
   cp saved_model/v9-t.cut_full_integer_quant.tflite yolov9-t_512x288_argmax.tflite
fi
...
if [ -f yolov9-t_512x288_argmax.tflite ]; then
   tflite_preprocess yolov9-t_512x288_argmax.tflite  --scale 255
fi
...
if [ -f yolov9-t_512x288_argmax.pre.tflite ]; then
   tflite_postprocess yolov9-t_512x288_argmax.pre.tflite   -p YOLO_ARG_MAX
fi
...
if [ -f yolov9-t_512x288_argmax.pre.post.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov9-t_512x288_argmax.pre.post.tflite  -o yolov9-t_512x288_argmax_V1000_ncomp.vnnx
fi
...
if [ -f yolov9-t_512x288_argmax_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov9-t_512x288_argmax_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.512.288.jpg -v 8 -t 0.2 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov9-t_512x288_argmax_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.512.288.jpg OBJECT_DETECT'
fi
