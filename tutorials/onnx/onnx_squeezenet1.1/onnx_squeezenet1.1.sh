...
if [ -f squeezenet1.1-7.onnx ]; then
   if ! echo "497ad0774f4e0b59e4f2c77ae88fcdfc squeezenet1.1-7.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the onnx_squeezenet1.1 model file as the expected checksum does not match.\n The model source can be found at: https://github.com/onnx/models/tree/main/validated/vision/classification/squeezenet.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
