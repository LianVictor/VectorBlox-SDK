...
if [ ! -f yolov8n_512x288.tflite ]; then
   if [ ! -f yolov8n.tflite ]; then
       ...
       yolo export model=yolov8n.pt format=tflite int8 imgsz=288,512 || true
       cp yolov8n_saved_model/yolov8n_full_integer_quant.tflite yolov8n_512x288.tflite
   fi
...
if [ -f yolov8n_512x288_V1000_ncomp.vnnx ]; then
    ...
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8n_512x288_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.512.288.jpg OBJECT_DETECT'
fi
