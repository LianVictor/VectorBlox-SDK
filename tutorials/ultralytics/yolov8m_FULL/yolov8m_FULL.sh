...
if [ ! -f yolov8m_FULL.tflite ]; then
    ...
    cp yolov8m_saved_model/yolov8m_full_integer_quant.tflite yolov8m_FULL.tflite
fi


if [ -f yolov8m.pt ]; then
   if ! echo "02adb7544860a8d62a4cede5b789851d yolov8m.pt" | md5sum -c; then
       echo -e "\n There is an issue with the yolov8m_FULL model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/ultralytics/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov8m_FULL.tflite ]; then
   tflite_preprocess yolov8m_FULL.tflite  --scale 255
fi
...
if [ -f yolov8m_FULL.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov8m_FULL.pre.tflite  -o yolov8m_FULL_V1000_ncomp.vnnx
fi
...
if [ -f yolov8m_FULL_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov8m_FULL_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8m_FULL_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT_FULL'
fi
