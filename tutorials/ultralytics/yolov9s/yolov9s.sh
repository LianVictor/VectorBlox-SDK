...
echo "Checking for yolov9s files..."
...
if [ -f yolov9s.tflite ]; then 
   ...
   mv yolov9s.0.tflite yolov9s.cut.tflite 
fi
...
if [ -f yolov9s.cut.tflite ]; then
   tflite_preprocess yolov9s.cut.tflite  --scale 255
fi
...
if [ -f yolov9s.cut.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov9s.cut.pre.tflite  -o yolov9s_V1000_ncomp.vnnx
fi
...
if [ -f yolov9s_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov9s_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov9s_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT'
fi
