...
echo "Checking for yolov9t files..."
...
if [ -f yolov9t.tflite ]; then 
   ...
   mv yolov9t.0.tflite yolov9t.cut.tflite 
fi
...
if [ -f yolov9t.cut.tflite ]; then
   tflite_preprocess yolov9t.cut.tflite  --scale 255
fi
...
if [ -f yolov9t.cut.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov9t.cut.pre.tflite  -o yolov9t_V1000_ncomp.vnnx
fi
...
if [ -f yolov9t_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov9t_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov9t_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT'
fi
