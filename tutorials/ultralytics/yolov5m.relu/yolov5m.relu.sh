...
if [ -f ultralytics.yolov5m.relu.onnx ]; then
   if ! echo "198d677d6f1fd710554b6ef7072ec7c6 ultralytics.yolov5m.relu.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the yolov5m.relu model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/yolov5.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov5m.relu.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov5m.relu.pre.tflite  -o yolov5m.relu_V1000_ncomp.vnnx
fi
...
if [ -f yolov5m.relu_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov5m.relu_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -j yolov5m.json -v 5 -l coco.names -t 0.25 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov5m.relu_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg YOLOV5'
fi
