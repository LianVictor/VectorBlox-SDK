...
if [ -f yolov3-tinyu.pt ]; then
   if ! echo "0e52accbe02df47f7a7396e945c55376 yolov3-tinyu.pt" | md5sum -c; then
       echo -e "\n There is an issue with the yolov3-tinyu_FULL model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/ultralytics/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov3-tinyu_FULL_V1000_ncomp.vnnx ]; then
    ...
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov3-tinyu_FULL_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT_FULL'
fi
