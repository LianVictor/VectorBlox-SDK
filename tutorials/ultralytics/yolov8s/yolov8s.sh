...
echo "Checking for yolov8s files..."
...
if [ -f yolov8s.tflite ]; then 
   ...
   mv yolov8s.0.tflite yolov8s.cut.tflite 
fi
...
if [ -f yolov8s.cut.tflite ]; then
   tflite_preprocess yolov8s.cut.tflite  --scale 255
fi
...
if [ -f yolov8s.cut.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov8s.cut.pre.tflite  -o yolov8s_V1000_ncomp.vnnx
fi
...
if [ -f yolov8s_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov8s_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8s_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT'
fi
