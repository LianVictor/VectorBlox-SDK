...
if [ -f yolov5m.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov5m.pre.tflite  -o yolov5m_V1000_ncomp.vnnx
fi
...
if [ -f yolov5m_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov5m_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -j yolov5m.json -v 5 -l coco.names -t 0.25 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov5m_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg  '
fi
