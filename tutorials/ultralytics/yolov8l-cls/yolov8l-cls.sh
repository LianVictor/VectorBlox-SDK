...
echo "Checking for yolov8l-cls files..."

# model details @ https://github.com/ultralytics/ultralytics/
if [ ! -f yolov8l-cls.onnx ]; then
    yolo export model=yolov8l-cls.pt format=onnx
fi
...
if [ -f yolov8l-cls.pt ]; then
   if ! echo "535d2ed411c32c5fc9184c36d725769e yolov8l-cls.pt" | md5sum -c; then
       echo -e "\n There is an issue with the yolov8l-cls model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/ultralytics/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ ! -f yolov8l-cls.tflite ]; then
   echo "Running ONNX2TF..."
   onnx2tf -cind images $VBX_SDK/tutorials/coco2017_rgb_norm_20x224x224x3.npy [[[[0.,0.,0.]]]] [[[[1.,1.,1.]]]] \
-i yolov8l-cls.onnx \
--output_signaturedefs \
--output_integer_quantized_tflite
   cp saved_model/yolov8l-cls_full_integer_quant.tflite yolov8l-cls.tflite
fi
...
if [ -f yolov8l-cls.tflite ]; then
   tflite_preprocess yolov8l-cls.tflite  --scale 255
fi
...
if [ -f yolov8l-cls.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t yolov8l-cls.pre.tflite  -o yolov8l-cls_V1000_ncomp.vnnx
fi
...
if [ -f yolov8l-cls_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py yolov8l-cls_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8l-cls_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
