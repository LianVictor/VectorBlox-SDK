...
echo "Checking for yolov8n-cls files..."
...
if [ -f yolov8n-cls.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov8n-cls.pre.tflite  -o yolov8n-cls_V1000_ncomp.vnnx
fi
...
if [ -f yolov8n-cls_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py yolov8n-cls_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8n-cls_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
