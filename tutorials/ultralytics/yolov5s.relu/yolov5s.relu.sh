...
if [ -f ultralytics.yolov5s.relu.onnx ]; then
   if ! echo "5a31a8d025bd323dadbc6d273bbfade0 ultralytics.yolov5s.relu.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the yolov5s.relu model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/yolov5.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
