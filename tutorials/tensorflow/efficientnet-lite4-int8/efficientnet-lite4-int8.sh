...
if [ -f efficientnet-lite4.tar.gz ]; then
   if ! echo "9042a6c9166875a4a5fdc82f56bcb64a efficientnet-lite4.tar.gz" | md5sum -c; then
       echo -e "\n There is an issue with the efficientnet-lite4-int8 model file as the expected checksum does not match.\n The model source can be found at: https://www.kaggle.com/models/google/efficientnet/tensorFlow1/b4-classification/1.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f efficientnet-lite4-int8.cut.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t efficientnet-lite4-int8.cut.pre.tflite  -o efficientnet-lite4-int8_V1000_ncomp.vnnx
fi
...
if [ -f efficientnet-lite4-int8_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py efficientnet-lite4-int8_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model efficientnet-lite4-int8_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
