...
echo "Checking for efficientnet-lite0-int8 files..."
...
if [ ! -f efficientnet-lite0-int8.tflite ]; then
    wget -q --no-check-certificate https://storage.googleapis.com/cloud-tpu-checkpoints/efficientnet/lite/efficientnet-lite0.tar.gz
...
if [ -f efficientnet-lite0.tar.gz ]; then
   if ! echo "703c66bc3d3463bd15c00c5e2ec535eb efficientnet-lite0.tar.gz" | md5sum -c; then
       echo -e "\n There is an issue with the efficientnet-lite0-int8 model file as the expected checksum does not match.\n The model source can be found at: https://www.kaggle.com/models/google/efficientnet/tensorFlow1/b0-classification/1.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f efficientnet-lite0-int8.tflite ]; then 
   echo "Cutting graph" 
   tflite_cut efficientnet-lite0-int8.tflite -c 0 62
   mv efficientnet-lite0-int8.1.tflite efficientnet-lite0-int8.cut.tflite 
fi
...
if [ -f efficientnet-lite0-int8.cut.tflite ]; then
   tflite_preprocess efficientnet-lite0-int8.cut.tflite  --scale 255
fi
...
if [ -f efficientnet-lite0-int8.cut.pre.tflite ]; then
    echo "Generating VNNX for V1000 ncomp configuration..."
    vnnx_compile -s V1000 -c ncomp -t efficientnet-lite0-int8.cut.pre.tflite  -o efficientnet-lite0-int8_V1000_ncomp.vnnx
fi
...
if [ -f efficientnet-lite0-int8_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py efficientnet-lite0-int8_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model efficientnet-lite0-int8_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
