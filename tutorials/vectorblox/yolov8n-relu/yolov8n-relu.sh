...
if [ -f yolov8n-relu.pt ]; then
   if ! echo "baf933d8cd52492d1dd0e96155317822 yolov8n-relu.pt" | md5sum -c; then
       echo -e "\n There is an issue with the yolov8n-relu model file as the expected checksum does not match.\n The model source can be found at: https://github.com/ultralytics/ultralytics/.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f yolov8n-relu.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t yolov8n-relu.pre.tflite  -o yolov8n-relu_V1000_ncomp.vnnx
fi
...
if [ -f yolov8n-relu_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/yoloInfer.py yolov8n-relu_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg -v 8 -l coco.names 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model yolov8n-relu_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/dog.jpg OBJECT_DETECT_FULL'
fi
