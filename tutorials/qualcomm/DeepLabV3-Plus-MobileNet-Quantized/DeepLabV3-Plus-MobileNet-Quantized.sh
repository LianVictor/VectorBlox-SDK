...
if [ -f DeepLabV3-Plus-MobileNet.onnx ]; then
   if ! echo "192e1e451be90dfa1b71e42717db6f47 DeepLabV3-Plus-MobileNet.onnx" | md5sum -c; then
       echo -e "\n There is an issue with the DeepLabV3-Plus-MobileNet-Quantized model file as the expected checksum does not match.\n The model source can be found at: https://aihub.qualcomm.com/mobile/models/deeplabv3_plus_mobilenet.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f DeepLabV3-Plus-MobileNet-Quantized.pre.post.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t DeepLabV3-Plus-MobileNet-Quantized.pre.post.tflite  -o DeepLabV3-Plus-MobileNet-Quantized_V1000_ncomp.vnnx
fi
...
if [ -f DeepLabV3-Plus-MobileNet-Quantized_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/segmentation.py DeepLabV3-Plus-MobileNet-Quantized_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/A0PQ76.jpg --inj 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model DeepLabV3-Plus-MobileNet-Quantized_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/A0PQ76.jpg  '
fi
