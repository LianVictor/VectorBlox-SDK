...
if [ -f ResNet101Quantized.tflite ]; then
   if ! echo "53c5a976b3f5dbf2534fea8fa6c696f3 ResNet101Quantized.tflite" | md5sum -c; then
       echo -e "\n There is an issue with the ResNet101Quantized model file as the expected checksum does not match.\n The model source can be found at: https://huggingface.co/qualcomm/ResNet101.\n If the model information has changed, please update this script and re-run the tutorial."
       exit 1
   fi
fi
...
if [ -f ResNet101Quantized.pre.tflite ]; then
    ...
    vnnx_compile -s V1000 -c ncomp -t ResNet101Quantized.pre.tflite  -o ResNet101Quantized_V1000_ncomp.vnnx
fi
...
if [ -f ResNet101Quantized_V1000_ncomp.vnnx ]; then
    echo "Running Simulation..."
    python $VBX_SDK/example/python/classifier.py ResNet101Quantized_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg 
    echo "C Simulation Command:"
    echo '$VBX_SDK/example/sim-c/sim-run-model ResNet101Quantized_V1000_ncomp.vnnx $VBX_SDK/tutorials/test_images/oreo.jpg CLASSIFY'
fi
