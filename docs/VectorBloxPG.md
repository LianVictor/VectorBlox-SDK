...
The VectorBlox™ programmer’s guide covers both the installation and use of the VectorBlox Accelerator SDK. It includes the steps needed to convert a high-level model description to a graph that runs on the CoreVectorBlox IP or on the bit-accurate simulator.  
...
- VBX Binary File Generation with vnnx_compile
...
The flow diagram below shows how the SDK converts a Convolutional Neural Network (CNN) model into a VectorBlox binary file (.vnnx)

The five starting paths, represented by the white nodes, depend on the model source (PyTorch, OpenVINO, ONNX, TensorFlow, and TensorFlow Lite). Models downloaded with OpenVINO are processed through OpenVINO's model optimizer and PINTO's openvino2tensorflow tool to create a TFLite model. PINTO converts models in ONNX or PyTorch format to TFLite using the onnx2tf tool. TensorFlow models can be converted directly to TFLite. Models already in quantized INT8 TFLite format proceed directly to the vnnx_compile step.**

As mentioned with PyTorch, we can go straight to TFLite with onnx2tf, or we can use the [VectorBlox Compression Library](https://github.com/Microchip-Vectorblox/VectorBlox-Compression) GitHub repository to retrain for compression. Please note that this is an advanced tactic used to increase model performance. When producing a compressed model from the compression library, please vnnx compile with COMP or UCOMP.
...
Before using the SDK, install dependencies, create a Python virtual environment (venv), and set several environment variables. To set up the Python virtual environment (venv) and initialize several environment variables, perform the following steps:

1. Ensure that the user is working in an Ubuntu 20.04, 22.04, or 24.04 environment with at least Python 3.10.
...
4. Create and activate the Python virtual environment and set the environment variables with the following command:
...
For converting to TF Lite, users need their own NumPy array files for TF Lite calibration. SDK tutorials download the necessary NumPy calibration data. Users can leverage VectorBlox SDK's generate_npy tool to format their data into a NumPy array file.

The following sections briefly describe each VectorBlox flow tool, along with links to its documentation. The tools are available from the command line within the VBX Python environment.
...
The OpenVINO mo command converts and optimizes models for inference. It converts models from various source frameworks to the OpenVINO Intermediate Representation (IR). The tool further optimizes models for inference by removing training-time layers, such as dropout, and applying layer fusion. The following are examples of the command’s usage. The full documentation is available in [OpenVINO Toolkit](https://docs.openvino.ai/2025/index.html).
...
This section describes how to convert the TF Lite model generated by the SDK toolkit commands into a binary file. The vnnx_compile command is available from the command line within the VBX Python environment. For more details on compression and size configurations, please refer to the CoreVectorBlox IP Handbook.
...
The VectorBlox binary file generation tool processes the TF Lite model and produces a quantized binary file that runs on both hardware and the simulator. To use the tool, provide the input TF Lite model and hardware size configuration (V250, V500, or V1000). If you use custom pre-processing or post-processing code, you can specify the starting and ending nodes for the VNNX graph. By default, tutorials use V1000 with no compression.

>Note: Please make sure the core configuration matches the compression and size config used in vnnx_compile. For example, if you use a no-compression VectorBlox core with hardware size V1000. Please run vnnx_compile with the -s V1000 and -c ncomp arguments.

<details>

<summary><b>Display Usage</b></summary>
...
The following scripts run the indicated models with the Python simulator and print the inference results. They also generate an annotated output image.
